...
export class AdaptationActionsClimateMonitoringComponent implements OnInit {
	...
	buildPayload() {
		const context = {
			progress_log: {
				action_status: this.form.value.formArray[0]
					.actionStatusCorrespondingReportingPeriodCtrl,
				progress_monitoring: this.form.value.formArray[0]
					.progressMonitoringRecordedClimateActionsCtrl
			},
			indicator_monitoring: {
				start_date: this.datePipe.transform(
					this.form.value.formArray[1].reportPeriodStartCtrl,
					"yyyy-MM-dd"
				),
				end_date: this.datePipe.transform(
					this.form.value.formArray[1].reportPeriodEndtCtrl,
					"yyyy-MM-dd"
				),
