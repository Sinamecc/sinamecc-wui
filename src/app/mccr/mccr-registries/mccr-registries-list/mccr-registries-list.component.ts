...
export class MccrRegistriesDataSource extends DataSource<any> {
	...
	connect(): Observable<MccrRegistry[]> {
		const mccrRegistriesPromise: Observable<
			MccrRegistry[]
		> = this.service.mccrRegistries();
		mccrRegistriesPromise.subscribe(
			mccrRegistries => (this.mccrRegistries = mccrRegistries)
		);
...
export class MccrRegistriesListComponent implements OnInit {
	...
	delete(uuid: string) {
		...
		this.service.deleteMccrRegistry(uuid).subscribe(() => {
			...
			this.translateService
				.get("Sucessfully deleted element")
	...
	addReview(uuid: string) {
		...
		const route = this.service
			.mapRoutesStatuses(uuid)
	...
	hasPermProvider() {
		return Boolean(
			this.authenticationService.credentials.permissions.all ||
