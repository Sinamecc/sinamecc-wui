...
import { GeographicLevel } from '@app/ppcn/interfaces/geographicLevel';
import { Ovv, PpcnNewFormData } from '@app/ppcn/ppcn-new-form-data';
...
export class PpcnService {
  ...
  submitNewPpcnForm(context: any): Observable<Response> {
    ...
    return this.httpClient.post(routes.submitNewPpcn(), formData, httpOptions).pipe(
      map((body: any) => {
        const response = {
          statusCode: 200,
          id: body.id,
  ...
  submitUpdatePpcnForm(
    ...
    return this.httpClient.put(routes.submitUpdatePpcn(id), formData, httpOptions).pipe(
      map((body: any) => {
        const response = {
          statusCode: 200,
          id: body.id,
  ...
  geographicLevel(lang: string): Observable<GeographicLevel[]> {
    ...
    return this.httpClient.get(routes.getGeographicLevel(lang), httpOptions).pipe(
      map((body: any) => {
        return body;
      })
    );
  ...
  newPpcnFormData(levelId: string, lang: string): Observable<PpcnNewFormData> {
    ...
    return this.httpClient.get(routes.seededFormData(levelId, lang), httpOptions).pipe(
      map((body: any) => {
        return body;
      })
    );
  ...
  subsectors(sector: string, lang: string): Observable<SubSector[]> {
    ...
    return this.httpClient.get(routes.subsectors(sector, lang), httpOptions).pipe(
      map((body: any) => {
        return body;
      })
    );
  ...
  submitPpcnNewFile(context: any): Observable<Response> {
    ...
    if (fileList.length > 0) {
      ...
      return this.httpClient.post(routes.submitNewFilePPCN(), formData, httpOptions).pipe(
        map((body: any) => {
          const response = {
            statusCode: 200,
  ...
  getPpcnReviewStatuses(): Observable<PpcnNewFormData> {
    ...
    return this.httpClient.get(routes.ppcnAvailableStatuses(), httpOptions).pipe(
      map((body: any) => {
        return body;
      })
    );
  ...
  private buildFormData(
    ...
    gasRemoval['removal_cost_currency'] = context.formArray[6].costRemovalInventoryValueCtrl;
    gasRemoval['total'] = context.formArray[6].totalremovalsCtrl;
    gasRemoval['removal_descriptions'] = context.formArray[6].removalProjectDetailCtrl;
    ...
    organization_classification['reduction'] =
      validateListReduction.indexOf(context.formArray[2].recognitionCtrl) >= 0 ? reduction : null;
    organization_classification['carbon_offset'] =
      validateListCompensation.indexOf(context.formArray[2].recognitionCtrl) >= 0 ? carbonOffset : null;
