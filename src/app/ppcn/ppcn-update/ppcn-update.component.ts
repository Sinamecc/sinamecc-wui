...
import { Ovv, PpcnNewFormData, RecognitionType, RequiredLevel, Sector, SubSector } from '@app/ppcn/ppcn-new-form-data';
...
export class PpcnUpdateComponent implements OnInit {
  ...
  private createForm() {
    ...
    this.formValues = forkJoin(
      ...
      this.initFormData(),
      (formOptions: PpcnNewFormData, formData: Ppcn) => {
  ...
  private loadFormData(): Observable<Ppcn> {
    return this.service.getPpcn(this.id, this.i18nService.language.split('-')[0]).pipe(
      finalize(() => {
        this.isLoading = false;
      })
    );
