...
export class PpcnUpdateComponent implements OnInit {
	...
	loadActivityForm(ppcn: any): FormGroup {
		return this.formBuilder.group({
			activityCtrl: [
				ppcn["gei_organization"]["gei_activity_types"]["0"]["activity_type"]
			],
			sectorCtrl: [
				ppcn["gei_organization"]["gei_activity_types"]["0"]["sector"]
			],
			subSectorCtrl: [
				ppcn["gei_organization"]["gei_activity_types"]["0"]["sub_sector"]
			]
		});
	}
	addItems(): void {
	...
	private initFormData(): Observable<Ppcn> {
		const ppcn$ = this.loadFormData().pipe(
			tap(ppcn => {
				...
				this.requiredFormId = +ppcn["organization_classification"][
					"required_level"
				]["id"];
				this.recognitionFormId = +ppcn["organization_classification"][
					"recognition_type"
				]["id"];
				if (ppcn["gei_organization"] != null) {
					...
					this.report_date_start =
						ppcn["gei_organization"]["report_date_start"];
					this.report_date_end = ppcn["gei_organization"]["report_date_end"];
					this.sectorFormId = +ppcn["gei_organization"]["gei_activity_types"][
						"0"
					]["sector"]["id"];
					...
					this.activity_type =
						ppcn["gei_organization"]["gei_activity_types"]["0"][
							"activity_type"
						];
				}
				...
				const ppcnArray = this.formBuilder.array([
					this.formBuilder.group({
						...
						representativeNameCtrl: [
							ppcn["organization"]["representative_name"],
							Validators.required
						],
						telephoneCtrl: [
							ppcn["organization"]["phone_organization"],
							Validators.compose([Validators.required, Validators.minLength(8)])
						],
					...
					this.formBuilder.group({
						contactNameCtrl: [
							ppcn["organization"]["contact"]["full_name"],
							Validators.required
						],
						positionCtrl: [
							ppcn["organization"]["contact"]["job_title"],
							Validators.required
						],
						emailFormCtrl: [
							ppcn["organization"]["contact"]["email"],
							Validators.email
						],
						phoneCtrl: [
							ppcn["organization"]["contact"]["phone"],
							Validators.compose([Validators.required, Validators.minLength(8)])
						]
					}),
					this.formBuilder.group({
						requiredCtrl: [
							ppcn["organization_classification"]["required_level"]["id"],
							Validators.required
						],
						recognitionCtrl: [
							ppcn["organization_classification"]["recognition_type"]["id"],
							Validators.required
						]
					}),
					this.formBuilder.group({
						baseYearCtrl: [
							ppcn["gei_organization"]["base_year"],
							Validators.required
						],
						reportYearCtrl: [
							ppcn["gei_organization"]["report_year"],
							Validators.required
						],
						ovvCtrl: [ppcn["gei_organization"]["ovv"], Validators.required],
						implementationEmissionDateCtrl: [
							ppcn["gei_organization"]["emission_ovv_date"],
							Validators.required
						]
					}),
					this.formBuilder.group({
