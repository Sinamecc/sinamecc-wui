...
export class PpcnSource extends DataSource<any> {
	...
	connect(): Observable<Ppcn[]> {
		this.ppcns$ = this.service.reRoutePpcn(
...
export class PpcnListComponent implements OnInit {
	...
	loadData() {
		this.service
			.reRoutePpcn(this.i18nService.language.split("-")[0])
			.subscribe((ppcns: Ppcn[]) => {
	...
	addReview(uuid: string) {
		...
		if (route) {
			...
			this.router.navigate([`ppcn/${uuid}/review/status/new`], {
				replaceUrl: true
			});
		}
	}
